...
from page.web_ui.accounting.page_salary import SalaryPage
...
class TestSalary(
    ...
    def test_import_salary_detail(self):
        ...
        with allure.step('打开工资菜单'):
            ...
            self.send_filepath_to_input(
                f'{get_project_path()}/template/accounting/salary/工资表导入-002.xls')
            self.click_import_area_buttons('导入')
    ...
    @allure.title('工资表切换期间合计数错误')
    def test_switch_period_sum_error(self):
        company = GetYamlData().get_company('proj_R20231011-029')
        with allure.step('登录'):
            self.login(company=company)
...
class TestSalaryExport(
    ...
    def test_salary_template(self):
        ...
        with allure.step('对比excel文件差异'):
            assert check_excel_diff(f'{get_project_path()}/template/accounting/salary/工资表导入-新.xls',
    ...
    def test_export(self):
        ...
        with allure.step('打开工资菜单'):
            ...
            self.rename_downloaded_file(tmp_filename)
        with allure.step('对比excel文件差异'):
            assert check_excel_diff(f'{get_project_path()}/template/accounting/salary/工资表-003.xls',
                                    f'{get_project_path()}\\download_tmp\\{self.test_id}\\{tmp_filename}')
