...
from page.web_ui.accounting.page_closure import (ClosurePage,
...
class TestClosure(
    ...
    def test_closure_and_reverse_closure_no_data(self):
        ...
        with allure.step('点击会计菜单'):
            self.click_accounting_menu('结账')
            self.switch_to_closure_frame()
        with allure.step('检查当前期间'):
            if '2023年第1期' != self.get_closure_current_period():
                self.click_buttons('反结账')
                self.closure_reverse_period_select('2023年1期')
                self.click_accounting_focus_table_buttons('确定')
                self.switch_to_default_content()
                self.scroll_to_top()
                assert '恭喜您，反结账成功！' in self.get_all_floating_tip()
                assert '2023年第1期' == self.get_accounting_current_period()
                self.switch_to_closure_frame()
        with allure.step('结账'):
            ...
            assert '2023年第2期' == self.get_accounting_current_period()
            self.switch_to_closure_frame()
            assert '2023年第2期' == self.get_closure_current_period()
            self.switch_to_default_content()
        with allure.step('反结账'):
            self.click_accounting_menu('结账')
            self.switch_to_closure_frame()
            ...
            assert '2023年第1期' == self.get_accounting_current_period()
            self.switch_to_closure_frame()
            assert '2023年第1期' == self.get_closure_current_period()
            self.switch_to_default_content()

    @allure.tag('【管家】2024年01月')
    @allure.tag('R20231203-004')
    @allure.title('银行存款/现金期末检查不校验综合本位币')
    def test_closure_check_bank_or_cash(self):
        company = GetYamlData().get_company('proj_R20231203-004')
        with allure.step('登录'):
            self.login(company=company)
        with allure.step('点击会计菜单'):
            self.click_accounting_menu('结账')
            self.switch_to_closure_frame()
        with allure.step('检查当前期间'):
            if '2022年第10期' != self.get_closure_current_period():
                self.click_buttons('反结账')
                self.closure_reverse_period_select('2022年10期')
                self.click_accounting_focus_table_buttons('确定')
                self.switch_to_default_content()
                assert '恭喜您，反结账成功！' in self.get_all_floating_tip()
                assert '2022年第10期' == self.get_accounting_current_period()
                self.switch_to_closure_frame()
        with allure.step('结账'):
            self.click_buttons('结转到下期')
            self.click_accounting_focus_table_buttons('确定')
            self.switch_to_default_content()
            assert '恭喜您，结账到2022年11期成功！' in self.get_all_floating_tip()
            assert '2022年第11期' == self.get_accounting_current_period()
            self.switch_to_closure_frame()
            assert '2022年第11期' == self.get_closure_current_period()
            self.switch_to_default_content()
        with allure.step('反结账'):
            self.click_accounting_menu('结账')
            self.switch_to_closure_frame()
            ...
            assert '2022年第10期' == self.get_accounting_current_period()
            self.switch_to_closure_frame()
            assert '2022年第10期' == self.get_closure_current_period()
            self.switch_to_default_content()

    @allure.title('结账检查银行存款/现金余额不小于零')
    def test_closure_check_bank_or_cash_balance_above_0(self):
        print(self.__class__.__name__, self.__class__.__module__, self._testMethodName)
        # company = GetYamlData().get_company('company_accounting_closure_009')
        # with allure.step('登录'):
        #     self.login(company=company)
        # with allure.step('点击会计菜单'):
        #     self.click_accounting_menu('结账')
        ...
        #     self.click_buttons('结转到下期')
        #     self.click_accounting_focus_table_buttons('确定')
        #     assert '结转到下期失败!银行存款余额小于0，请检查科目余额' in self.get_text_from_accounting_focus_table_sys_tips()
        #
...
class TestProductionCosts(
    ...
    @allure.tag('【管家】2024年01月')
    @allure.tag('R20231204-064')
    @allure.title('预置结转模板生成凭证跳转')
    def test_default_template_generate_voucher_requisition_raw_materials_click_finish_num_is_0(self):
        company = GetYamlData().get_company('proj_R20231204-064')
        with allure.step('登录'):
            self.login(company=company)
        with allure.step('点击会计菜单'):
            self.click_accounting_menu('结账')
        ...
        with allure.step('结转生产成本-生成凭证'):
            ...
            self.switch_to_production_costs_frame()
            self.wait(1)
            self.click_production_costs_next_step()
            assert '4957222.94' == self.get_text_from_production_costs_total_jz()
            self.click_production_costs_show_wg()
            assert '4957222.94' == self.get_text_from_production_costs_total_jz()
...
class TestSellingCosts(
    ...
    @allure.tag('R20231101-003')
    @allure.title('库存商品科目本期发生额为0无法结转销售成本')
    def test_selling_cost_amount_is_zero(self):
        company = GetYamlData().get_company('proj_R20231101-003')
        with allure.step('登录'):
            self.login(company=company)
        with allure.step('点击会计菜单'):
            self.click_accounting_menu('结账')
        ...
        with allure.step('结转销售成本-生成凭证'):
            ...
            self.switch_to_default_content()
            self.switch_to_selling_costs_frame()
        with allure.step('按收入比例结转'):
            self.selling_costs_click_buttons('按收入比例结转')
            assert '库存商品科目本期发生额为0无法结转销售成本' in self.get_all_floating_tip()
...
class TestClosureCheck(
    ...
    def test_closure_check_report_tips_check_accounts_receivable_closing_balance_tips(self):
        ...
        with allure.step('重新检测'):
            ...
            tips_label = '应收账款期末余额'
            self.click_report_formula_tips_by_label(tips_label)
            assert '1131 应收账款期末余额 3,372,243.68 = 3,372,243.68 利润表本年累计营业收入 = 0.00' \
                   in self.get_text_from_report_formula_tips_p_value(tips_label)
    ...
    def test_closure_check_report_tips_check_closing_balance_of_advance_receivables_tips(self):
        ...
        with allure.step('重新检测'):
            ...
            tips_label = '预收账款期末余额'
            self.click_report_formula_tips_by_label(tips_label)
            assert '利润表本年累计营业收入 = 0.00' in self.get_text_from_report_formula_tips_p_value(
                tips_label)
    ...
    def test_closure_check_report_tips_check_profit_ratio_tips(self):
        ...
        with allure.step('重新检测'):
            ...
            tips_label = '利润率'
            self.click_report_icon_tips_by_label(tips_label)
            assert '来自利润表本年累计、本月金额：利润率=利润总额/营业收入' == \
                   self.get_text_from_report_icon_tips_value(tips_label)
