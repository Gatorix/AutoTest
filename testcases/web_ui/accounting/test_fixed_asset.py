...
from page.web_ui.accounting.page_fixed_asset import (FixedAssetClassPage,
                                                     FixedAssetCardPage,
...
class TestFixedAssetCard(
    ...
    @allure.tag('R20231113-029')
    @pytest.mark.p1
    @allure.title('卡片编辑页面和卡片列表残值存在尾差')
    def test_edit_card_and_card_list_diff(self):
        company = GetYamlData().get_company('proj_R20231113-029')
        with allure.step('登录'):
            self.login(company=company)
        with allure.step('点击会计菜单'):
            self.click_accounting_menu('资产', '卡片')
        ...
        with allure.step('录入卡片信息'):
            num = random_string_generator()
            self.add_card(num, amount='2485.10')
            assert '124.26' == self.get_text_from_line_edit('预计残值')
            assert '新增卡片成功' in self.get_all_floating_tip()
        with allure.step('检查列表残值'):
            self.switch_to_default_content()
            self.close_tab_by_tab_name('新增卡片')
            self.switch_to_fixed_asset_card_frame()
            self.click_card_buttons('刷新')
            assert '124.26' == self.get_text_from_list_texts_by_table_head(num, 'residual')
        with allure.step('删除卡片'):
            self.click_buttons_in_line_by_num(num, '删除')
            self.click_conform_buttons('确定')
...
class TestFixedAssetDepreciationDetail(
    ...
    @allure.tag('R20230807-024')
    @allure.title('卡片折旧明细表中双击月折旧的金额，会出现500的英文报错')
    def test_double_click_detail_report_line_500(self):
        ...
        with allure.step('点击会计菜单'):
            self.click_accounting_menu('资产', '折旧明细表')
            self.switch_to_fixed_asset_depreciation_detail_frame()
            self.doubleclick_fixed_asset_depreciation_sum_line_td()
            self.switch_to_default_content()
            assert not self.is_top_tab_visible('修改卡片')
...
class TestFixedAssetExport(
    ...
    def test_export_card(self):
        ...
        with allure.step('对比excel文件差异'):
            assert check_excel_diff(f'{get_project_path()}/template/accounting/fixed_asset/资产卡片列表-002.xls',
...
@pytest.mark.accounting
@pytest.mark.accounting_fixed_asset
@pytest.mark.accounting_fixed_asset_print
@allure.epic('会计')
@allure.feature('固定资产')
@allure.story('打印')
class TestFixedAssetPrint(
    LoginPage,
    AccountingHomePage,
    FixedAssetDepreciationSummaryPage,
    FixedAssetDepreciationDetailPage
    ...
    @allure.tag('【管家】2023-12-18')
    @allure.tag('R20231121-039')
    @allure.title('折旧明细表新增打印设置')
    def test_depreciation_detail_print_settings(self):
        company = GetYamlData().get_company('company_accounting_smart_bookkeeping_050')
        with allure.step('登录'):
            self.login(company=company)
        with allure.step('点击会计菜单'):
            self.click_accounting_menu('资产', '折旧明细表')
            self.switch_to_fixed_asset_depreciation_detail_frame()
        with allure.step('点击打印PDF'):
            self.click_buttons('print')
            self.switch_to_print_settings_frame()
            assert self.is_element_visible(self.print_settings_label_text())

    @allure.tag('【管家】2023-12-18')
    @allure.tag('R20231121-039')
    @allure.title('折旧汇总表新增打印设置')
    def test_depreciation_summary_print_settings(self):
        company = GetYamlData().get_company('company_accounting_smart_bookkeeping_050')
        with allure.step('登录'):
            self.login(company=company)
        with allure.step('点击会计菜单'):
