...
from page.web_ui.manager.page_business import BusinessServicePage
from page.web_ui.manager.page_contract import ContractExpirePage, ContractPage, ContractInvoicePage, \
    ContractChangeRecordPage
from page.web_ui.manager.page_items import ItemsPage
from page.web_ui.page_login import LoginPage
from page.web_ui.manager.page_home import ManagerHomePage
from page.web_ui.manager.page_customer import CustomerPage, LostManagePage
...
class TestContract(LoginPage,
    ...
    def test_termination_of_the_contract_empty_reason(self):
        ...
        with allure.step('输入代账服务信息'):
            self.input_bookkeeping_detail('服务开始日期', '2022-11')
            self.input_bookkeeping_detail('服务结束日期', '2024-11')
        with allure.step('保存合同'):
            self.click_save_button()
        with allure.step('验证浮动提示'):
            assert '新增合同成功' in self.get_tip_text()
