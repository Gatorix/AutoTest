...
from page.manager.page_agency import AgencyAccountPage, ReceiveInvoiceRecordPage, ReceiveInvoicePage, BookkeepingPage
...
class TestAgency(LoginPage,
    ...
    def test_dispatch_transfer(self):
        ...
        with allure.step('转派工'):
            ...
            # 点击转派工后，选择xx部门xx职员
            self.click_dispatch_department_select_button('行政部')  # 根据老部门定位新部门，默认选择第一个
            # 选择XX职员（默认选择第一职员）
    ...
    @allure.title('创建账套并删除-村集体经济组织会计制度')
    def test_create_account_set_accounting_standards_for_village(self):
        account_name = random_string_generator()
        with allure.step('登录'):
            self.login()
            self.close_popup()
        ...
        with allure.step('创建账套_填写启用期间'):
            ...
            self.input_enable_month('11')  # 编写启用期间：xxx年xx期，如2022年11期
            self.select_tax_type('一般纳税人')  # 选择【纳税性质】:一般纳税人、小规模纳税人
            self.select_accounting_standards('村集体经济组织会计制度')
        with allure.step('创建账套_点击开始创建'):
            self.click_create_button('开始创建')  # 后跳转至会计页面
            assert account_name == self.get_company_name()
            self.driver.close()
