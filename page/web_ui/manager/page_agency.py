...
from base.manager.base_agency import (AgencyAccountBase,
                                      ...
                                      BookkeepingBase,
                                      FileTaxesBase,
                                      ReceiveInvoiceBase, GatherInvoiceBase)
...
class AgencyAccountPage(AgencyAccountBase, ServiceAuditBase, BaseTestCase):
    ...
    def click_page_popups_bottom_button(self, button_name, types):
        if self.is_element_visible(self.page_popups_bottom_buttons(button_name, types)):
            self.click(self.page_popups_bottom_buttons(button_name, types))
            self.wait(2)
        else:
            self.click(self.invoice_aria_bottom_buttons('确定', types))
...
class GatherInvoicePage(GatherInvoiceBase, BaseTestCase):
    def switch_to_gather_frame(self):
        self.switch_to_frame(self.gather_frame())

    def click_invoice_tab(self):
        self.click(self.invoice_tab())

    def click_in_line_buttons_by_invoice_name(self, name, button):
        self.click(self.in_line_buttons_by_invoice_name(name, button))

    def is_conform_box_cancel_buttons_visible(self):
        return self.is_element_visible(self.conform_box_cancel_buttons())
