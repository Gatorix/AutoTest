...
class ClosurePage(BaseTestCase, ClosureBase):
    ...
    def close_closure_popups(self):
        if self.is_element_visible(self.closure_write_off_tips()):
            self.click(self.closure_write_off_tips())
    ...
    def get_closure_current_period(self):
        return self.get_text(self.closure_current_period())

    def closure_reverse_period_select(self, period_text):
        self.click(self.closure_reverse_period_select_input_trigger())
        self.click(self.closure_reverse_period_select_drop_down_items(period_text))
...
class ProductionCostsPage(BaseTestCase, ProductionCostsBase):
    ...
    def click_production_costs_next_step(self):
        self.click(self.production_costs_next_step())

    def click_production_costs_show_wg(self):
        self.click(self.production_costs_show_wg())

    def get_text_from_production_costs_total_jz(self):
        return self.get_text(self.production_costs_total_jz())
...
class SellingCostsPage(BaseTestCase, SellingCostsBase):
    ...
    def selling_costs_click_buttons(self, button):
        button_kv = {
            '按收入比例结转': 'srCarryOver'
        }
        self.click(self.selling_costs_buttons(button_kv.get(button)))
